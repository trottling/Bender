...
def Load_Styles(self):
    ...
    if self.app_theme in (None, "", "Default (Dark)", "Default (Light)", "Custom"):
        ...
        self.ui.setStyleSheet(open(file=GetRelPath(self, f"assets\\qss\\Material{self.theme_to_load}.qss"), mode="r").read())
        ...
        if os.path.isfile(f"{self.appdir}\\saved_qss\\{self.app_theme}"):
            ...
            try:
                self.ui.setStyleSheet(open(file=GetRelPath(self, f"{self.appdir}\\saved_qss\\{self.app_theme}"), mode="r").read())
