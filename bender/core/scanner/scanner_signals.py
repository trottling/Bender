...
def connect_scanner_signals(self):
    ...
    self.scanner.stat_signal.connect(lambda res: update_work_page_stat(self, res))
    self.scanner.label_System_image_setStyleSheet_signal.connect(lambda item: self.ui.label_System_image.setPixmap(QPixmap(get_rel_path(self, r"assets\\images\\" + item))))
    ...
    self.scanner.frame_sys_bitness_setStyleSheet_signal.connect(lambda item: self.ui.frame_sys_bitness.setPixmap(QPixmap(get_rel_path(self, item))))
    ...
    self.scanner.FillAllAppsList_signal.connect(lambda item: fill_all_apps_list(self, item))
    self.scanner.ReportApps_signal.connect(lambda item: report_apps(self, item))
    self.scanner.FillLocalPorts_signal.connect(lambda item: fill_local_ports(self, item))
    self.scanner.FillExtPorts_signal.connect(lambda item: fill_ext_ports(self, item))
    self.scanner.FillDriversList_signal.connect(lambda item: fill_drivers_list(self, item))
    self.scanner.ReportDrivers_signal.connect(lambda item: report_drivers(self, item))
    self.scanner.FillKBList_signal.connect(lambda data_inst, data_miss: fill_kb_list(self, data_inst, data_miss))
    self.scanner.ReportKB_signal.connect(lambda item: report_kb(self, item))
    self.scanner.UpdateWorkPageStat_signal.connect(lambda item: update_work_page_stat(self, item))
